...
type (
	CashPositionEntryCashCount struct {
		...
		OrganizationID uuid.UUID     `gorm:"type:uuid;not null;index:idx_org_branch_cpe_cash_count" json:"organization_id"`
		Organization   *Organization `gorm:"foreignKey:OrganizationID;constraint:OnDelete:CASCADE,OnUpdate:CASCADE;" json:"organization,omitempty"`
		BranchID       uuid.UUID     `gorm:"type:uuid;not null;index:idx_org_branch_cpe_cash_count" json:"branch_id"`
		...
		BillAmount float64 `gorm:"type:decimal" json:"bill_amount"`
		Quantity   int     `gorm:"type:int" json:"quantity"`
		Amount     float64 `gorm:"type:decimal" json:"amount"`
	}

	CashPositionEntryCashCountResponse struct {
	...
	CashPositionEntryCashCountRequest struct {
