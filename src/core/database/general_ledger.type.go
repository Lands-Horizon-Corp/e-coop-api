...
const (
	...
	GeneralLedgerSourcePayment            GeneralLedgerSource = "payment" // collection
	...
	GeneralLedgerSourceCheckVoucher       GeneralLedgerSource = "check voucher" // disbursements
	GeneralLedgerSourceLoan               GeneralLedgerSource = "loan"
	GeneralLedgerSourceSavingsInterest    GeneralLedgerSource = "savings interest" // savings intereset comp.
	...
	GeneralLedgerSourceOtherFund          GeneralLedgerSource = "other fund"
)
...
type (
	...
	GeneralLedger struct {
		...
		EntryDate time.Time `gorm:"type:timestamp;not null;default:now();index:idx_ledger_pagination,priority:1" json:"entry_date"`
		...
		RevolvingFundID *uuid.UUID     `gorm:"type:uuid" json:"revolving_fund_id,omitempty"`
		RevolvingFund   *RevolvingFund `gorm:"foreignKey:RevolvingFundID;constraint:OnDelete:RESTRICT,OnUpdate:CASCADE;" json:"revolving_fund,omitempty"`
		Particular      string         `gorm:"type:varchar(255)" json:"particular"`
	}
	...
	GeneralLedgerResponse struct {
		...
		RevolvingFund   *RevolvingFundResponse `json:"revolving_fund,omitempty"`
	...
	ChangeORRequest struct {
		ORFrom string `json:"or_from" validate:"required"`
		ORTo   string `json:"or_to" validate:"required"`
	}
)
